...
const SetInitialPassword: React.FC<SetInitialPasswordProps> = ({ onSetSuccess }) => {
    ...
    const handlePasswordUpdate = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setSuccessMessage('¡Contraseña creada con éxito! Ahora puedes iniciar sesión.');
        } catch (err: any) {
    ...
    return (
        <div className="flex items-center justify-center min-h-screen bg-gray-100 dark:bg-taller-dark">
            <div className="relative w-full max-w-md p-8 space-y-6 bg-white dark:bg-gray-800 rounded-2xl shadow-lg text-center">
                ...
                <h1 className="text-3xl font-bold text-taller-dark dark:text-taller-light">Crea tu Contraseña</h1>
                
                {successMessage ? (
                    <div className="space-y-4">
                         <p className="text-green-600">{successMessage}</p>
                         <button onClick={onSetSuccess} className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-taller-primary hover:bg-taller-secondary focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-taller-primary">
                            Ir a Iniciar Sesión
                        </button>
                    </div>
                ) : (
                    <>
                        <p className="text-taller-gray dark:text-gray-400">
                            ¡Bienvenido! Para completar tu registro, crea una contraseña segura.
                        </p>
