...
import { ChevronLeftIcon, XMarkIcon, TrashIcon, ChevronDownIcon, PlusIcon, CameraIcon, UserIcon, PhoneIcon, EnvelopeIcon, CheckIcon, SparklesIcon } from '@heroicons/react/24/solid';
...
const CrearClienteModal: React.FC<CrearClienteModalProps> = ({ onClose, onSuccess, clienteToEdit }) => {
    ...
    return createPortal(
        <div className="fixed inset-0 z-[100] flex justify-end">
            ...
            <div ref={modalRef} className={`bg-white dark:bg-gray-800 text-taller-dark dark:text-taller-light w-full h-[100dvh] sm:h-full sm:max-h-none sm:max-w-xl sm:rounded-none flex flex-col overflow-hidden relative z-10 transform transition-all duration-500 ease-out ${isVisible ? 'translate-x-0' : 'translate-x-full'}`}>
                <div className="safe-top-padding-portal px-4 pb-4 border-b dark:border-gray-700 flex justify-between items-center bg-gray-50 dark:bg-gray-900/50 flex-shrink-0 glass-modal-header">
                    ...
                    <h2 className="font-bold text-lg text-taller-dark dark:text-taller-light truncate">{isEditMode ? 'Editar Cliente' : 'Nuevo Cliente'}</h2>
