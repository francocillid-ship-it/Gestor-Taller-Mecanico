...
const AutoExpandingInput: React.FC<{
    ...
    return (
        <textarea
            ...
            className={`w-full bg-transparent text-sm font-medium focus:outline-none resize-none overflow-hidden transition-all duration-200 block text-center ${isCategory ? 'font-extrabold uppercase text-taller-dark dark:text-taller-light placeholder-gray-400' : 'text-taller-dark dark:text-taller-light'
...
const CrearTrabajoModal: React.FC<CrearTrabajoModalProps> = ({ onClose, onSuccess, onDataRefresh, clientes, trabajoToEdit, initialClientId }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const laborCost = cleanPartes
                .filter(p => p.isService && !p.isCategory && !p.clientPaidDirectly)
                .reduce((s, p) => s + (p.cantidad * p.precioUnitario), 0);
    ...
    return createPortal(
        <div className="fixed inset-0 z-[100] flex justify-end">
            <div ref={backdropRef} className={`fixed inset-0 bg-black/60 backdrop-blur-sm transition-opacity duration-500 ${isVisible ? 'opacity-100' : 'opacity-0'}`} onClick={handleClose} />
            <div ref={modalRef} className={`bg-white dark:bg-gray-800 text-taller-dark dark:text-taller-light w-full app-height-mobile sm:h-full sm:max-h-none sm:max-w-xl sm:rounded-none flex flex-col overflow-hidden relative z-10 transform transition-all duration-500 ease-out ${isVisible ? 'translate-x-0' : 'translate-x-full'}`}>
                <div className="safe-top-padding-portal px-4 pb-4 border-b dark:border-gray-700 flex justify-between items-center bg-gray-50 dark:bg-gray-900/50 flex-shrink-0 glass-modal-header">
                ...
                <div className="p-4 border-t dark:border-gray-700 bg-white dark:bg-gray-800 flex gap-2 flex-shrink-0 glass-modal-footer">
                    {isEditMode && (
