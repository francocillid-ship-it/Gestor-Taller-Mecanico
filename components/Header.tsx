...
const Header: React.FC<HeaderProps> = ({ tallerName, logoUrl, onMenuClick, showMenuButton, searchQuery, onSearchChange }) => {
    ...
    useEffect(() => {
        if (isSearchExpanded && mobileInputRef.current) {
            ...
            const timer = setTimeout(() => {
                mobileInputRef.current?.focus();
            }, 100); // Un poco más de delay para Safari iOS
    ...
    return (
        <header className="relative pt-[env(safe-area-inset-top)] bg-white dark:bg-gray-800 shadow-md dark:shadow-none dark:border-b dark:border-gray-700 z-[70] flex-shrink-0">
